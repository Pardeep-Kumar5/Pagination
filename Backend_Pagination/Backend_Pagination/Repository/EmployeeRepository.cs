...
namespace Backend_Pagination.Repository
{
    public class EmployeeRepository : IEmployeeRepository
    {
        ...
        public IEnumerable<Employee> GetEmployeesByFilter(string nameFilter, string addressFilter, int? salaryFilter)
        {
            using (var connection = new SqlConnection(_connectionstring))
            {
                connection.Open();
                using (var command = new SqlCommand("GetEmployeesByPageWithFilter", connection))
                {
                    command.CommandType = CommandType.StoredProcedure;

                    command.Parameters.AddWithValue("@NameFilter", string.IsNullOrEmpty(nameFilter) ? (object)DBNull.Value : nameFilter);
                    command.Parameters.AddWithValue("@AddressFilter", string.IsNullOrEmpty(addressFilter) ? (object)DBNull.Value : addressFilter);
                    command.Parameters.AddWithValue("@SalaryFilter", salaryFilter.HasValue ? (object)salaryFilter.Value : DBNull.Value);
